...
#define MAX_INCOMING_HITS 10000 
#define MAX_LOGGED_E_HITS 5000 // Maximum E hits to print in the log file
#define GRAV 9.81 
...
typedef struct {
    ...
    int hitsTakenInStep;
    IncomingHit e_hits_log[MAX_LOGGED_E_HITS]; // Added to store E ship logs
} SimResult;
...
TypeInfo_E info_ETypes[5] = {
    {"EA", "1936A-class Destroyer",   "SK C/34 naval gun",        0.08, 20.0, 0.0, 0.0, 0, 0, 0.0},
    {"EB", "Gabbiano-class Corvette", "L/47 dual-purpose gun",    0.06, 30.0, 0.0, 0.0, 0, 0, 0.0},
    {"EC", "Matsu-class Destroyer",   "Type 89 dual-purpose gun", 0.07, 25.0, 0.0, 0.0, 0, 0, 0.0},
    {"ED", "F-class Escort Ships",    "SK c/32 naval gun",        0.05, 50.0, 0.0, 0.0, 0, 0, 0.0},
    {"EE", "Japanese Kaibokan",       "(4.7 inch) naval guns",    0.04, 70.0, 0.0, 0.0, 0, 0, 0.0}
};
...
void resetBattlefield() {
    for (int i = 0; i < noEscort; i++) E_ships[i] = original_E_ships[i];
}
...
int compareAttackTargets(const void *a, const void *b) {
    AttackTarget *t1 = (AttackTarget *)a; AttackTarget *t2 = (AttackTarget *)b;
...
int compareIncomingHits(const void *a, const void *b) {
    IncomingHit *h1 = (IncomingHit *)a; IncomingHit *h2 = (IncomingHit *)b;
    if (h1->impact_time < h2->impact_time) return -1;
    if (h1->impact_time > h2->impact_time) return 1;
    return 0;
}

// ============= TIMELINE BATTLE SIMULATION LOGIC ===================
...
SimResult simulateBattleStep(double bx, double by, double minAngleAllowedB, double *b_total_damage) {
    ...
    result.escortHitCount = 0; result.hitsTakenInStep = 0;
    ...
    for (int i = 0; i < noEscort; i++) {
        ...
        if (dist <= maxRangeB) {
            ...
            if (canEscortHitB(i, bx, by)) {
                ...
                double dps = E_ships[i].typeInfo.impactpower / E_ships[i].typeInfo.reloadTime;
                targets[t_count].threat_score = dps / eft; 
            } else { targets[t_count].threat_score = 0.0; }
            t_count++;
        }
    }
    ...
    double t_destroyed[MAX_ESCORTS];
    for (int i = 0; i < noEscort; i++) t_destroyed[i] = 999999.0;

    for (int i = 0; i < t_count; i++) {
        int e_idx = targets[i].e_index;
        t_destroyed[e_idx] = b_fire_time + targets[i].ft_B_to_E;
        targets[i].scheduled_fire_time = b_fire_time; 
        b_fire_time += reloadTimeB;
    }

    IncomingHit in_hits[MAX_INCOMING_HITS];
    int in_hit_count = 0;
    ...
    for (int i = 0; i < noEscort; i++) {
        ...
        if (canEscortHitB(i, bx, by)) {
            ...
            double e_reload = E_ships[i].typeInfo.reloadTime;
            double t_fire_e = 0.0;
            
            while (t_fire_e < t_destroyed[i]) {
                if (in_hit_count < MAX_INCOMING_HITS) {
                    in_hits[in_hit_count] = (IncomingHit){ i, t_fire_e, eft, t_fire_e + eft, E_ships[i].typeInfo.impactpower };
                    in_hit_count++;
                }
                t_fire_e += e_reload;
            }
    ...
    qsort(in_hits, in_hit_count, sizeof(IncomingHit), compareIncomingHits);

    double b_sink_time = -1.0;
    for (int i = 0; i < in_hit_count; i++) {
        // Log the incoming hit into our result structure!
        if (result.hitsTakenInStep < MAX_LOGGED_E_HITS) {
            result.e_hits_log[result.hitsTakenInStep] = in_hits[i];
        }

        *b_total_damage += in_hits[i].impact;
        result.hitsTakenInStep++;
        
        if (*b_total_damage >= 1.0) {
            result.battleshipSunk = 1;
            result.sunkByEscortIndex = E_ships[in_hits[i].e_array_idx].index;
            b_sink_time = in_hits[i].impact_time;
            break;
        }
    }
    ...
    for (int i = 0; i < t_count; i++) {
        if (result.battleshipSunk && targets[i].scheduled_fire_time >= b_sink_time) continue; 
        
        int e_idx = targets[i].e_index;
        E_ships[e_idx].alive = 0; 
        
        result.hits[result.escortHitCount] = (HitRecord){ 
            E_ships[e_idx].index, targets[i].dist, targets[i].ft_B_to_E, 
            targets[i].b_angle, targets[i].b_velocity, targets[i].scheduled_fire_time 
        };
        result.escortHitCount++;
        
        double impact_t = targets[i].scheduled_fire_time + targets[i].ft_B_to_E;
        if (!result.battleshipSunk && impact_t > max_sim_time) max_sim_time = impact_t;
    }

    if (!result.battleshipSunk && in_hit_count > 0) {
        double last_e_hit = in_hits[in_hit_count-1].impact_time;
        if (last_e_hit > max_sim_time) max_sim_time = last_e_hit;
    }
...
void appendStepResultToFile(FILE *file, int step, int bx, int by, double minAngle, SimResult result, double b_total_damage) {
    ...
    for (int i = 0; i < result.escortHitCount; i++) {
        ...
        fprintf(file, "  -> Destroyed Escort #%d | Fired at: %.1fs | Impact at: %.1fs\n", 
                h.escortIndex, h.fireTime, h.fireTime + h.flightTime);
    }

    fprintf(file, "\n--- Number of bullets that hit B in this iteration: %d ---\n", result.hitsTakenInStep);
    
    // NEW: Detailed log of Incoming E Ship Hits
    int print_limit = (result.hitsTakenInStep < MAX_LOGGED_E_HITS) ? result.hitsTakenInStep : MAX_LOGGED_E_HITS;
    for (int i = 0; i < print_limit; i++) {
        IncomingHit eh = result.e_hits_log[i];
        fprintf(file, "  -> Hit by Escort #%d | Fired at: %.2fs | Impact at: %.2fs | Damage Caused: %.1f%%\n",
                E_ships[eh.e_array_idx].index, eh.t_fire, eh.impact_time, eh.impact * 100.0);
    }
    if (result.hitsTakenInStep > MAX_LOGGED_E_HITS) {
        fprintf(file, "  ... (Only first %d hits logged to save space)\n", MAX_LOGGED_E_HITS);
    }
...
void runSimulation(int simMode) {
    ...
    fprintf(file, "========== SIMULATION %d RESULTS (Part 2-B) ==========\n\n", simMode);
    ...
    for (int step = 0; step < k_iterations; step++) {
        ...
        printf("Iteration %d: %s | Damage Taken: %.1f%% | B Hit %d times | E Destroyed: %d\n", 
               step + 1, result.battleshipSunk ? "SUNK!" : "Survived", b_total_damage * 100.0, result.hitsTakenInStep, result.escortHitCount);
